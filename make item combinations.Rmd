...
so sequence is A N1 B N2 C N3 D N4 A N5 B N6 C N7 D N8 etc. This means if we have 3 rounds of A/B/C/D per session, we have 3 x 8  =24 items per session. For 4 rounds 4 x 8 = 32 items per session. We will go for 5 rounds, ie 5 x 8 = 40 items per session (20 of each type). We can always cut down if this is too many.

Need to fix power with N items PER TYPE and check we have that right.
In each session, for now will go for 5 rounds using 4 repeated items (each followed by a novel item), this gives 40 items in total per session.

Need enough items for 4 sessions, i.e. 40 x 4 items altogether.  
The repeated item, by definition, is taken from a repeated set of 3, but it would be good to have alternative versions of novel items (A-C) so that we avoid any effects due to specific stimuli being used.  However, my attempt to do that in one go just created havoc, so here we just will create one set.
If we want a different set later, we would need to change set.seed() above and then run the script to generate a completely fresh set of items.
------------------------------------------------------------------------
## Difficulty levels: this is between subjects
We need items for two levels that use 2-item combinations, and for 2 levels that use 3-item combinations (we dropped one original level)
Levels now are; 
#Table 1 (as per registered report)
Level 1	Level 2	Level 3	Level 4  
Open boxes	      2	    2	      2	        2  
N items to select	1	    2	      2	        2  
N pre-fixed items	1	    0	      1	        1  

Sample sentences	 
“The boat is below the scarf”	  
“The shirt is above the door”	  
“The hen is between the star and the flag”	  
“The bed is next to the tree and below the jug"  

If we just create 4 (levels) x 4 (blocks) x 36 3-item combinations, we can then just use the 3rd item as a distractor for levels 1-2. This is 576 combinations.
...
itemtype<-c('R01','R02','R03','R04','N01','N02','N03','N04','N05','N06','N07','N08','N09','N10','N11','N12','N13','N14','N15','N16','N17','N18','N19','N20')
...
myn <- nrow(permlist)
counter <- 0
nitems <- 40 #NB we will use this later on when compiling Gorilla sheet. Note that because of repeated items, the N trios is less than this.
#Currently this is N items per session, with 4 repeated items presented 5 times plus one novel item for each
...
This seemed to work!  
We have 384 trios, with no overlap in items between items within 4-items apart. I think we also have avoided having conflicting pairs of picturs together in an item.
...
itemorder <- c(1,5,2,6,3,7,4,8,1,9,2,10,3,11,4,12,1,13,2,14,3,15,4,16,1,17,2,18,3,19,4,20,1,21,2,22,3,23,4,24)
...
for (mysession in 1:4){
  for (mylevel in 1:4){
    ...
    myspreadsheet <- data.frame(matrix(NA,ncol=length(spreadsheetcols),nrow=nitems))
      ...
      for (m in 1:maxm[mylevel]){ #X, Y, Z, Y1 and Z1
        ...
        if(length(w)>0){
          ...
          if(myitem==m){ #item to place in grid
            ...
            #Remove placed item from AnswerChoice and AnswerCell
            if(myitem==1){
              # bizarrely, for character cell cannot just do <- NA!!
               myspreadsheet$Answer1Cell[n]<- is.na(myspreadsheet$Answer1Cell[n])
              myspreadsheet$Answer1Choice[n]<-NA
        
            }
                if(myitem==2){
              myspreadsheet$Answer2Choice[n]<-NA
              myspreadsheet$Answer2Cell[n]<- is.na(myspreadsheet$Answer2Cell[n])
            }
  ...
  myspreadsheet$Answer1Cell <- paste0(LETTERS[mytempcol%%10],(round(mytempcol/10,0)))
    w<-which(mytempcol/10==round(mytempcol/10),0) #these are rows where tempcol ends in zero
  myspreadsheet$Answer1Cell[w]<-NA
  
  mytempcol <- myspreadsheet$Answer2Cell
  myspreadsheet$Answer2Cell <- paste0(LETTERS[mytempcol%%10],(round(mytempcol/10,0)))
      w<-which(mytempcol/10==round(mytempcol/10),0) #these are rows where tempcol ends in zero
  myspreadsheet$Answer2Cell[w]<-NA
  
  if(mylevel>2){
    mytempcol <- myspreadsheet$Answer3Cell
    myspreadsheet$Answer3Cell <- paste0(LETTERS[mytempcol%%10],(round(mytempcol/10,0)))
        w<-which(mytempcol/10==round(mytempcol/10),0) #these are rows where tempcol ends in zero
  myspreadsheet$Answer3Cell[w]<-NA
  }
 
  #Where the answercell corresponds to a fixed item, code ends in zero. Need to just delete these.
...
```{r soundGenerator}
makesounds<-0 #just make this chunk optional as it crashes if you don't have json properly installed
if (makesounds==1){
