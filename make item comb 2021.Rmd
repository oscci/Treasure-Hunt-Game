...
#Examples of frames 1,2,3,4 are saved 
thisframe <- 0 
Nperframe <- 6
for (myframe in c(1,2,3,4)){
  thisframe<-thisframe+1
  myfilename <- paste0('./sheets/sheetSession1frame_',myframe,'.csv')
  testbit <- read.csv(myfilename,stringsAsFactors=F)
  if(myframe==1){testsheet<-testbit[1:(4*Nperframe),]}
  if(myframe>1){
    theserows<-((thisframe-1)*Nperframe+1):(thisframe*Nperframe)
    testsheet[theserows,]<-testbit[1:Nperframe,]
  }
