...
for (myn in 1:length(w)){
  worder <- sample(1:4)
gdf[w[myn],(startcol-1+worder[1])] <- paste0("Item_",myn,'A.png')
gdf[w[myn],(startcol-1+worder[2])] <- paste0("Item_",myn,'B.png')
gdf[w[myn],(startcol-1+worder[3])] <- paste0("Item_",myn,'C.png')
gdf[w[myn],(startcol-1+worder[4])] <- paste0("Item_",myn,'D.png')
}
